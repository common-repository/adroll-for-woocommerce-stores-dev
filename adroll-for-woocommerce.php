...
class AdRoll_For_WooCommerce {
    ...
    public function __construct() {
        add_action('checkout_started', array($this, 'send_checkout_payload_callback'), 10, 1);
    ...
    public function add_action_links( $actions ) {
        $mylink = '<a href="' . $this->get_adroll_admin_url() . '">Configure</a>';
        array_unshift( $actions, $mylink );
        return $actions;
    }

    public function activated_plugin_action_callback($plugin) {
        // When plugin gets activated, we used to redirect users to the plugin's settings page
        // but we don't do that anymore because it triggers errors on 
        // QIT Activation tests - See .github/workflows/activation-test.yml
    ...
    private function serialize_product($product) {
        ...
        $serialized_product['name'] = $product->get_name();
    ...
    private function serialize_order_products($order) {
        ...
        foreach ($order->get_items() as $item) {
            $product = $this->adroll_get_product($item['product_id']);
            $serialized_product = $this->serialize_product($product);
            $quantity = floatval(isset($item['quantity']) ? $item['quantity'] : $item['qty']);
    ...
    private function is_conversion_page() {
        // Check if we are on a conversion page, and if it's the first time we are on this page.
        // This is needed to reduce the possibility of tracking the same order more than once.
        if (is_order_received_page() && (!is_null($this->order_id))) {
            $order = wc_get_order($this->order_id);

            $adroll_pixel = get_post_meta($this->order_id, 'adroll_pixel_'.$order->get_order_number(),true);
            if (empty($adroll_pixel)) {
                update_post_meta($this->order_id, 'adroll_pixel_'.$order->get_order_number(),1);
                return true;
            }
        }
        return false;
    ...
    public function get_global_vars() {
        ...
        $global_vars = array();
        $global_vars['adroll_current_page'] = $current_page;
        ...
        switch($current_page) {
            ...
            case 'product_page':
                ...
                break;
            case 'checkout_page':
                if (!WC()->session->__isset('checkout_uuid')) {
                    $uuid = uniqid();
                    WC()->session->set('checkout_uuid', $uuid);
                }

                $user = wp_get_current_user();
                $global_vars['shop_url'] = $user->user_url;
                $global_vars['adroll_products'] = $this->serialize_cart_products(WC()->cart->get_cart());
                $global_vars['cart_total'] = WC()->cart->total;
                $global_vars['cart_count'] = WC()->cart->get_cart_contents_count();
                do_action('checkout_started', $global_vars);
                break;
            case 'cart_page':
                $global_vars['adroll_products'] = $this->serialize_cart_products(WC()->cart->get_cart());
                break;
            case 'conversion_page':
                if (WC()->session->__isset('checkout_uuid')) WC()->session->__unset('checkout_uuid');
                ...
                $global_vars['adroll_email'] = (string)$order->get_billing_email();
                break;
            case 'home_page':
            default:
    ...
    public function send_checkout_payload_callback($vars) {
        $vars['advertisable'] = get_option('adroll_adv_eid');
        $vars['checkout_uuid'] = WC()->session->get('checkout_uuid');
        $vars['adroll_cookie'] = $_COOKIE['__adroll_fpc'];

        $payload = json_encode($vars);

        $url = "https://app.adroll.com/woocommerce/webhook/v1/checkout_created";
        $curl = curl_init();
        ...
        $headers = array(
            ...
            "X-Wc-Webhook-Topic: checkout.started",
            "X-Wc-Webhook-Event: start",
        );
        ...
        if (curl_errno($curl)) {
            echo "Caught error ". curl_errno($curl) . " while calling the checkout endpoint: ". curl_error($curl)."\n";
        }
